#!/usr/bin/env python3
"""Independent UV-slice verifier for the GF(2) <8,12,12> rank-705 scheme."""
import hashlib,json,sys
from pathlib import Path
N=8;M=P=12; EXPECTED=705; AB=N*M; BC=M*P; AC=N*P
def bits(x):
    while x:
...
def parse(path):
    ...
    return rank,terms
def main():
    path=Path(sys.argv[1] if len(sys.argv)>1 else 'matmul_8x12x12_rank705_gf2.txt')
